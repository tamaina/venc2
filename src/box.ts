import { Box, BoxParser, DataStream, Endianness } from "mp4box";
...
export function getDescriptionBoxEntriesFromTrak(trak: (typeof BoxParser)['box']['trak']) {
    return (trak as any)?.mdia?.minf?.stbl?.stsd?.entries as Box[] ?? [];
}
